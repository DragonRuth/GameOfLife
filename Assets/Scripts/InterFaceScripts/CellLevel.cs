...
public class CellLevel : CellTouchSript {
	private LevelsSceneController sceneManager;
	void OnMouseDown() {

		if (!IsPointerOverUIObject () && GameManager.instance.isTouchAllowed () && sceneManager.getCurrentlyActiveGenerationNum () != -1) {
			if (!stateAlive) {
				spriteRenderer.sprite = sprites [1];
				mainManager.bringToLifePointPV (1, sceneManager.getCurrentlyActiveGenerationNum (), pos);
	...
	public void setSxeneManager(LevelsSceneController scenMan) 
	{
